...
static uint32_t HIDD_sleep_handler(wiced_sleep_poll_type_t type )
{
    ...
    ret = hidd.p_app_sleep_handler(type);
#if 0
#if SLEEP_ALLOWED
    switch(type)
    {
...
// if support epds, we need to determine if it should enter epds or hidoff
  #ifdef SUPPORT_EPDS
           if (!hidd_link_is_disconnected() || !hidd.allowHIDOFF)
           {
               // enter ePDS
...
#ifdef HCI_TRACE_OVER_TRANSPORT
extern wiced_transport_buffer_pool_t* transport_pool;
